...
class LXNSClient:
    ...
    async def maimai_player(self, friend_code: str | int) -> dict | None:
        ...
        resp = await self._client.get(
            f"/api/v0/maimai/player/{friend_code}"
        )

        if resp.status_code == 200 and resp.json()["success"] is True and resp.json()["code"] == 200:
    ...
    async def update_maimai_player(self, player: dict) -> bool:
        ...
        resp = await self._client.post(
            "/api/v0/maimai/player",
            json=player
        )

        logger.debug(f"LXNSClient-update_maimai_player: \n"

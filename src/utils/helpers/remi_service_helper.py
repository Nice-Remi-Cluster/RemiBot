...
class RemiServiceHelper:
    ...
    async def user_get_uuid(self, bind_type: UserBindType, bind_content: str) -> str | None:
        """
        通过bind内容获取用户的uuid
    ...
    async def user_add_bind(self, uuid: str, bind_type: UserBindType, bind_content: str,
                            bind_name: str = None) -> _V1UserAddBindResponse | None:
        """
        添加一个新的用户账户绑定关系
    ...
    async def user_get_binds(self, uuid: str, bind_type: UserBindType, default: bool = False) -> list[_V1UserAddBindResponse] | None:
        """
        获取指定类型的所有绑定关系
    ...
    async def get_current_maimai_bind_info(self, uuid: str) -> MaimaiBindInfo | None:
        ...
        resp = await self._client.get(
            "/v1/maimai/get-current-maimai-bind",
            params={
                "uuid": uuid
            }
        )
        ...
        if resp.status_code == 200:
            ...
            return self._v1_maimai_get_current_maimai_bind_response_2_maimai_bind_info(result)
    ...
    async def switch_current_maimai_bind(self, uuid: str, mai_bind_name: str) -> MaimaiBindInfo | None:
        """
        切换用户当前maimai档案
        """
        resp = await self._client.get(
